...
from services.replay_service import (
    apply_header_transforms as _apply_header_transforms,
    extract_service_id as _extract_service_id,
    match_xml_template as _match_xml_template,
    replay_one as _replay_one,
    run_replay_job as _run_replay_job,
)
...
async def create_replay_job(
    ...
    effective_repeat_count = body.repeat_count or 1
    job = ReplayJob(
        id=str(uuid.uuid4()),
        total_count=tc.recording_count * effective_repeat_count,
        **body.model_dump(),
    )
    db.add(job)
    await db.commit()
